...
## このプロジェクトはLLMを用いて作成されています。コードの確認はしていますがご使用は自己責任でお願いいたします。

# [Download the latest version from here](https://github.com/MICHICOM/SuguAnki/releases/)
# [インストールはこちらから](https://github.com/MICHICOM/SuguAnki/releases/)
...
**For Windows:**
- `SuguAnki Setup X.X.X.exe` : Installer (Recommended)
- `SuguAnki-X.X.X-win.zip` : Portable version (No installation required)

**For Android:**
- `SuguAnki-Android-vX.X.X.apk` : Install directly on your Android device
...
1. **Prerequisite (AnkiConnect Setup):** 
   ...
   - **Restart Anki** to activate the add-on. *(Note: You must keep Anki running in the background whenever you use SuguAnki!)*
2. Launch SuguAnki and click the **Settings (⚙️)** in the header to register your Gemini API Key.
...
3. **Note:** Please create at least one Deck in AnkiDroid manually first, as decks cannot be created directly from SuguAnki on Android.
4. Open the SuguAnki app and grant the requested database access permissions. Your generated cards will be saved directly to your AnkiDroid!
...
**Windowsをお使いの方:**
- `SuguAnki Setup X.X.X.exe` : インストーラー版 (推奨)
- `SuguAnki-X.X.X-win.zip` : ポータブル版 (解凍するだけで使えます)

**Androidをお使いの方:**
- `SuguAnki-Android-vX.X.X.apk` : Android端末に直接インストールできます
...
1. **事前準備 (AnkiConnectの導入):** 
   ...
   - インストールが完了したら、必ず **Ankiを一度再起動** してください。（※SuguAnkiを使用する際は、常にAnkiを起動したままにしておく必要があります）
...
3. **注意:** Android版SuguAnkiからはデッキの新規作成ができない仕様のため、あらかじめAnkiDroid上で登録先のデッキを作成しておいてください。
4. SuguAnkiを開くとデータベースへのアクセス権限を求められるので「許可」します。これでAIが生成したカード情報が直接AnkiDroidに保存されるようになります！
...
### 📜 Credits & Licenses
- **SuguAnki** is created by MICHICOM and released under the [MIT License](LICENSE).
- **Capacitor**: [MIT License](https://github.com/ionic-team/capacitor/blob/main/LICENSE)
- **Google Fonts (Outfit, Plus Jakarta Sans)**: SIL Open Font License
